...
interface TripleToggleComponentProps {
    field1: FieldProps;
    field2: FieldProps;
    field3: FieldProps;
}
...
const TripleToggle : React.FC<TripleToggleComponentProps> = ({field1, field2, field3}) => {
    const [activeField, setActiveField] = useState<string>(field1.name);
    return (
        <div className="p-2 xbg-slate-800 flex space-x-2 rounded-md text-sm">
        <button onClick={()=>{
            field1.callback()
            setActiveField(field1.name)
        }} className={`px-3 py-1 w-1/3 rounded-md flex items-center justify-center
        text-center ${activeField == field1.name? 'bg-teal-900' : 'bg-gray-900'}`}><h1>{field1.name}</h1></button>
        <button onClick={()=> {
            field2.callback()
            setActiveField(field2.name)
        ...
        <button onClick={()=> {
            field3.callback()
            setActiveField(field3.name)
        }} className={`px-3 py-1 w-1/3 rounded-md flex items-center justify-center
        text-center ${activeField == field3.name? 'bg-teal-900' : 'bg-gray-900'}`}><h1>{field3.name}</h1></button>
        </div>
    );
}

export {Toggle, TripleToggle}